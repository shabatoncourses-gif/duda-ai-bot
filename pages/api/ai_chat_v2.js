...
function stripNavDropdown(text) {
  if (!text) return text;
  // עוגן על תחילת/סוף התפריט עצמו ("כל האזורים" ... "תרפיה וטיפול" — התחום
  // האחרון ברשימה) ולא על מה שבא *אחריו* בתוכן, כי זה משתנה בין עמודים
  // (לפעמים "חובות הלימודים", לפעמים "המוסדות המאושרים...", וכו').
  return text.replace(/(?:מצאו[^]{0,80}?)*כל האזורים[^]{0,2200}?תרפיה וטיפול\s*/g, '').trim();
}
