...
const SYSTEM_PROMPT =
  ...
  '⛔ כלל חובה/השלמה: אסור בהחלט לקבוע האם קורס מסוים מוכר כ"חובה" או "השלמה" — לא לתת תשובה חד-משמעית בשום מקרה. ההכרה תלויה בגורמים רבים: המוסד הספציפי, הפרופיל האישי של הגולש, ואישור קרן ההשתלמות. כשנשאלים על הכרה של קורס — (1) הצג קישור לדף כל הקורסים בתחום מה-context אם קיים — **אסור להמציא URL שאינו ב-context**. (2) הצג רשימת מוסדות שמציעים אותו מה-context. (3) הפנה כל אחד לדף המוסד. (4) ציין שמורה יכול לפנות לקרן ההשתלמות בבקשה מיוחדת להכרה בקורס ספציפי — גם אם אינו ברשימת הקורסים המאושרים. (5) אל תמציא מידע על "רשימות מאושרות" של קרן ההשתלמות — לא קיים מידע כזה ב-context.\n' +
  ...
  try {
    ...
    if (courseCount > 0 && coursesForClaude && coursesForClaude.length > 0) {
      ...
      if (getCourseDates(message) && courseCount <= 3) {
        ...
        // אם האזור המבוקש הוא "למידה מרחוק" (slug=online) — הרשימה כן מסוננת
        // (כי כל המוסדות ב-known_institutions של תחום "למידה מרחוק" הם מקוונים ממילא),
        // אז הפתיח צריך להיות חיובי ולא "לא הצלחנו לאתר".
