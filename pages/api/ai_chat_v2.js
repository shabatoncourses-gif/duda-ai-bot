...
const SYSTEM_PROMPT =
  ...
  '⛔ כלל חובה/השלמה: אסור בהחלט לקבוע האם קורס מסוים מוכר כ"חובה" או "השלמה" — לא לתת תשובה חד-משמעית בשום מקרה. ההכרה תלויה בגורמים רבים: המוסד הספציפי, הפרופיל האישי של הגולש, ואישור קרן ההשתלמות. כשנשאלים על הכרה של קורס — (1) הצג רשימת מוסדות שמציעים אותו מה-context. (2) הפנה כל אחד לדף המוסד. (3) ציין שמורה יכול לפנות לקרן ההשתלמות בבקשה מיוחדת להכרה בקורס ספציפי — גם אם אינו ברשימת הקורסים המאושרים. (4) אל תמציא מידע על "רשימות מאושרות" של קרן ההשתלמות — לא קיים מידע כזה ב-context.\n' +
