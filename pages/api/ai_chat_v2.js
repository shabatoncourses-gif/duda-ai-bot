...
const SYSTEM_PROMPT =
  ...
  'אם שאלו על מוסד ספציפי ואין עליו מידע ב-context — אל תתעכבו על העדר-המידע ואל תכתבו משפט-פתיחה על כך; עברו ישר להצגת מוסדות אחרים מה-context באותו תחום לימודים, למשל בפתיח כמו "בפורטל שבתון יש כמה אפשרויות טובות בתחום הזה:" ואז הרשימה. אל תציין שם המוסד החסר ב-URL.\n' +
  ...
  '⛔ כלל דו-משמעות "אלמד" בציון ימים: כשגולש/ת (מורה) מתאר/ת אילו ימים בשבוע פנוי/ה או תפוס/ה, בניסוח כמו "אני לא אלמד בימים X ו-Y" — הניסוח דו-משמעי בעברית כתובה בלי ניקוד: "אלמד" יכול להיות אֶלְמַד (אשתתף בקורסים/אלמד בעצמי) או אֲלַמֵּד (אעבוד/אורה בבית הספר). אלו שתי כוונות הפוכות: הראשונה מציינת ימים להימנע מהם בקורסים, השנייה מציינת בדיוק את הימים הפנויים שבהם הגולש/ת רוצה שהקורס יתקיים. אסור להניח בשקט איזו כוונה נכונה ולסנן/להמליץ בהתאם. יש לפתוח את התשובה במפורש בפרשנות שנבחרה (למשל: "הבנתי שבימים X ו-Y את/ה לא מלמד/ת בבית הספר — כלומר אלו הימים הפנויים שלך ללימודים, ולכן חיפשתי קורסים שמתקיימים דווקא בימים האלה. אם התכוונת להפך, ספר/י לי ואתקן.") כדי שהגולש/ת יוכל/תוכל לתקן מיד אם הפרשנות שגויה. כלל זה חל על כל ניסוח דומה שבו "אלמד"/"נלמד"/"ללמד" עלול להתפרש כלימוד-עצמי או כהוראה.\n' +
  ...
  '⛔ כלל ספציפי — פרטי קשר של קרנות/הסתדרות המורים/ארגון המורים/משרד החינוך: זו נקודת-כשל שחזרה בפועל, ולכן במפורש: מספרי טלפון, אימיילים ושמות אנשי-קשר בנושא הזה נכונים **רק** אם הם מופיעים מילה-במילה ב-context שסופק (contact-phones-shabaton / דף https://www.shabaton.online/phones_shabaton) — לעולם לא מהזיכרון הכללי שלך, גם אם הם נשמעים מוכרים או סבירים או נכונים. אם הפרט המבוקש (איזה מוקד, איזו קרן) לא ברור-וממוקד מהשאלה או לא נמצא ב-context שסופק — הפנו לעמוד המלא: [טלפונים וכתובות בשנת שבתון](https://www.shabaton.online/phones_shabaton), או לקבוצת הוואטסאפ של שבתון. אל תבחרו מוקד באופן שרירותי כשלא ברור איזה מהם רלוונטי.\n' +
  '⛔ ענה אך ורק על בסיס המידע שב-context. אסור להשתמש במידע כללי שלא מופיע ב-context. אם ה-context אינו מכיל מידע לשאלה — אל תשתמשו בניסוח של העדר-מידע; בקשו הבהרה ועברו ישר לפתרון, בדיוק כך: "איזה תחום לימוד מעניין אתכם? אשמח להציע קורסים מתאימים מפורטל שבתון." ואפשר להוסיף הפניה לקבוצת הוואטסאפ של שבתון לעזרה נוספת. — לעולם אל תכתוב ניסוחים כמו "אינו מופיע בתחזוקה הנוכחית" או "בסיס הנתונים" — אלו ניסוחים טכניים שלא מתאימים לגולש, ואל תקבע שמוסד "לא קיים" — פשוט הציעו לברר תחום-עניין ועברו לפתרון.\n' +
...
// ── "כמה גמולים אני מקבל/ת" — אין לפורטל תשובה על זה ─────────────────
// נצפה בפרודקשן: שאלה כמו "כמה גמולים אעלה אחרי דוקטורט" הפעילה בטעות
// את זיהוי-המוסד/detectInfoPages (כי ההודעה מכילה "דוקטורט"/"תואר
// שלישי" כמילות-מפתח) והחזירה כרטיס-מוסד לא-קשור על תוכניות דוקטורט,
// בלי לגעת בשאלה בפועל. מספר הגמולים תלוי בפרטים אישיים (תואר קודם,
// ותק, גמולים קיימים וכו') ונקבע על ידי משרד החינוך — זה לא מידע שקיים
// בפורטל שבתון, ואין טעם לנחש. מפנים ישירות לגורם המוסמך (המפקח על
// ההשתלמויות במשרד החינוך — אותו גורם שכבר מוזכר בהקשר דומה ב-QA של
// "רשימת המשימות בתום שנת שבתון").
const GEMUL_COUNT_QUESTION_RE = /כמה\s*(נקודות\s*)?גמול/;

function tryAnswerGemulCountQuestion(message) {
  const m = message || '';
  if (!GEMUL_COUNT_QUESTION_RE.test(m)) return null;

  return 'מספר הגמולים שמקבלים עבור תואר/השתלמות תלוי בפרטים אישיים ' +
    '(תואר קודם, ותק, גמולים קיימים ועוד) ונקבע על ידי משרד החינוך — ' +
    'זה לא מידע שקיים בפורטל שבתון.\n\n' +
    'הגורם המוסמך לענות על זה הוא **המפקח על ההשתלמויות במשרד החינוך**.\n\n' +
    `אם צריך כיוון נוסף, אפשר גם לשאול בקבוצת הוואטסאפ של שבתון: [אפשר לשאול בקבוצת הווטסאפ שבתון](${SHASH_WHATSAPP_LINK})`;
}

// ── פרטי-קשר לקרנות ההשתלמות / הסתדרות-ארגון המורים / משרד החינוך ────
// נבנה על סמך https://www.shabaton.online/phones_shabaton — נשלף ואומת
// באופן ידני (web_fetch על הדף החי), לא מהזיכרון הכללי של המודל. הבעיה
// שנצפתה לא הייתה שהמספרים שגויים (הם התבררו כנכונים!) — הבעיה היא
// שהם הגיעו מזיכרון-כללי בלתי-ניתן-לאימות ולא מ-context מסופק, מה
// שמפר את כלל "ענה אך ורק על בסיס context" גם כשהתוצאה נכונה במקרה.
// עכשיו זה מגיע מ-shabaton-phones.json בתור מקור אמיתי וממשי.
const UNION_CONTACT_RE = /הסתדרות\s*ה?מורים|ארגון\s*ה?מורים|קרן\s*(ה)?השתלמות|קרנות\s*(ה)?השתלמות/;

function tryAnswerUnionContactRequest(message) {
  const m = message || '';
  if (!UNION_CONTACT_RE.test(m)) return null;

  const data = loadJSON('shabaton-phones.json');
  if (!data || !data.contacts) return null;
  const byId = id => data.contacts.find(c => c.id === id);

  const mentionsHistadrut = /הסתדרות\s*ה?מורים/.test(m);
  const mentionsIrgun = /ארגון\s*ה?מורים/.test(m);
  const mentionsPedagogic = /תכנית\s*לימודים|מוסד(ות)?\s*לימוד|פדגוגי|קורס(ים)?|פוקוסינג|התמקדות/.test(m);
  const mentionsFinancial = /כספי|פיננסי|מענק|משיכ(ה|ת)|החזר\s*שכר/.test(m);
  const mentionsGemul = /גמול/.test(m);

  // אם אף אחד מהשניים (הסתדרות/ארגון) לא צוין בשם, ואף תחום (פדגוגי/כספי/
  // גמול) לא ברור — לא מנחשים איזה מוקד; מפנים לעמוד המלא בלבד.
  const anyUnionNamed = mentionsHistadrut || mentionsIrgun;
  const anyTopicClear = mentionsPedagogic || mentionsFinancial || mentionsGemul;
  if (!anyUnionNamed && !anyTopicClear) return null;

  const blocks = [];

  if (mentionsGemul) {
    const moe = byId('moe_hishtalmuyot');
    if (moe) blocks.push(`**בנושא גמולי השתלמות** — ${moe.note}\n[רשימת המפקחים על הפיתוח המקצועי](${moe.website})`);
  }

  // ברירת-מחדל: אם המשתמש/ת לא ציינ/ה קרן ספציפית, מציגים את שתיהן.
  const showHistadrut = mentionsHistadrut || !mentionsIrgun;
  const showIrgun = mentionsIrgun || !mentionsHistadrut;

  if (mentionsPedagogic || (!mentionsFinancial && !mentionsGemul)) {
    if (showHistadrut) {
      const c = byId('histadrut_pedagogic');
      if (c) blocks.push(`**${c.org} — ${c.desk}**\nטלפון: ${c.phone}\nשעות: ${c.hours}`);
    }
    if (showIrgun) {
      const c = byId('irgun_pedagogic');
      if (c) blocks.push(`**${c.org} — ${c.desk}**\nניתן לפנות ל${c.contactPerson}, טלפון: ${c.phone}, מייל: ${c.email}`);
    }
  }

  if (mentionsFinancial) {
    if (showHistadrut) {
      const c = byId('histadrut_financial');
      if (c) blocks.push(`**${c.org} — ${c.desk}**\nטלפון: ${c.phone} או ${c.phoneStar}\nשעות: ${c.hours}`);
    }
    if (showIrgun) {
      const c = byId('irgun_financial');
      if (c) blocks.push(`**${c.org} — ${c.desk}**\nטלפון: ${c.phone} או ${c.phoneStar}\nשעות: ${c.hours}`);
    }
  }
  ...
  blocks.push(`לכל פרטי הקשר המלאים: [טלפונים וכתובות בשנת שבתון](${data.sourceUrl})`);
  return blocks.join('\n\n');
}
...
function detectInfoPages(question) {
  ...
  const pages = [
    ...
    { kw: ['תכנון','תוכנית לימודים','תכנית לימודים','תוכנית הלימודים','תכנית הלימודים','טבלת עזר','הרכבת תוכנית','מתכנן','מתכננים','לתכנן','איך מרכיב','בניית תוכנית','תכנון לימודים','להרכיב תוכנית','מה ללמוד בשבתון','כמה שעות ללמוד','לימוד בשבתון','מוסדות מאושרים ללמוד','שעות השלמה','לימודי השלמה','ספורט בשבתון','שינוי תוכנית','אופק חדש בשבתון','גמול השתלמות','מסלול אישי','פרויקט אישי','לימודים בחו"ל'], urls: ['https://www.shabaton.online/shabaton-plan','https://www.shabaton.online/learning_programs_shabaton'] },
    { kw: ['שעות השלמה','לימודי השלמה','מוסדות מאושרים','נושאי השתלמות','ספורט בשבתון','שינוי תוכנית','תואר שלישי','דוקטורט','גמול השתלמות','מסלול אישי','פרויקט אישי','ישיבה','לימודים בחו"ל','לימודים בחול','תוכנית לימודים','תוכנית הלימודים','תכנית הלימודים','להרכיב תוכנית','הרכבת תוכנית','בניית תוכנית','מה ללמוד','כמה שעות','תכנית לימודים','לימוד בשבתון','מוסדות ללמוד','תוכנית הלימודים אושרה','תכנית הלימודים אושרה','אישור תוכנית הלימודים','אישור תכנית הלימודים','אישור הקרן','התוכנית אושרה','התכנית אושרה','אושרה על ידי הקרן','פורטל עובדי הוראה'], url: 'https://www.shabaton.online/learning_programs_shabaton' },
...
async function buildContext(message, history, precomputedQA, apiKey) {
  ...
  // ── הודעה שמזכירה הרבה תחומים-לימוד שונים בבת-אחת (3+) ──
  // נצפה בפרודקשן: "רפואה משלימה, יוגה, ספורט, תזונה, טיפול. ניתוח התנהגות"
  // (6 תחומים) ענתה רק על "ניתוח התנהגות" — כי מנגנון-ההתאמה-הרגיל תמיד
  // בוחר *מנצח אחד* (ה-keyword הכי ארוך/ספציפי). מנגנון-השילוב הקיים
  // (combinesWith) בנוי לזוג-שדות עם מילת-חיבור מפורשת ("וגם"/"או"), לא
  // לרשימה חופשית מופרדת-בפסיקים. כשמזהים 3+ תחומים נבדלים — כנראה גלישה
  // כללית, לא שאלה ממוקדת — ועדיף להציג קישורי-קטגוריה לכל תחום (עם הזמנה
  // לציין אזור לצמצום), במקום להחליט בשבילם איזה תחום "ניצח". פועל בכל
  // שלב בשיחה, לא רק בהודעה הראשונה — הדפוס הזה קורה גם באמצע שיחה.
  {
    const segments = message.split(/[,.;]|(?:\s+וגם\s+)|(?:\s+או\s+)/).map(s => s.trim()).filter(s => s.length >= 2);
    if (segments.length >= 3) {
      const seen = new Map();
      for (const seg of segments) {
        const fs = getFieldSlug(seg);
        if (fs && !seen.has(fs.name)) seen.set(fs.name, fs);
      }
      if (seen.size >= 3) {
        const links = [...seen.values()].map(fs =>
          `📚 [${fs.name}](https://www.shabaton.online/results-all/${fs.slug})`
        ).join('\n');
        return {
          context: `יש כאן כמה תחומים שונים — כדי לא "לבחור בשבילכם" תחום אחד, הנה קישור ישיר לכל תחום:\n\n${links}\n\n💡 בכל אחד מהקישורים אפשר גם לסנן לפי אזור המגורים — פשוט ציינו את שם היישוב או האזור בעת החיפוש באתר.`,
          isInfo: true, courseCount: seen.size, urlToTitle: {}
        };
      }
    }
  }
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // ── "כמה גמולים" — לפני INSTITUTION LOOKUP, כדי ש"דוקטורט"/"תואר
    // שלישי" בהודעה לא יחטפו לכרטיס-מוסד לא-קשור ──
    const gemulReply = tryAnswerGemulCountQuestion(message);
    if (gemulReply) {
      console.log('GEMUL COUNT QUESTION — direct reply');
      return res.json({ reply: gemulReply });
    }

    // ── פנייה לקרן/הסתדרות/ארגון המורים — נתונים אמיתיים מ-shabaton-phones.json,
    // לא מזיכרון כללי (גם אם הוא נשמע נכון) ──
    const unionContactReply = tryAnswerUnionContactRequest(message);
    if (unionContactReply) {
      console.log('UNION/FUND CONTACT REQUEST — direct reply');
      return res.json({ reply: unionContactReply });
    }
