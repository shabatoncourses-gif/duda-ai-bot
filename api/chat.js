...
export default async function handler(req, res) {
  ...
  try {
    ...
    const responseGeneral = await client.chat.completions.create({
      model: "gpt-4o-mini",
      messages: [
        {
          role: "system",
        ...
        { role: "user", content: `${contextGeneral}\n\n????: ${message}` }
      ],
      temperature: 0.5
    });
    ...
    return res.status(200).json({ reply: replyGeneral });
  } catch (error) {
    console.error("? ????? ?????:", error);
